...
from user.utils import Util, NO_ACTIVE_ACCOUNT
    ...
    if filtered_user_by_email.exists():
        ...
        if provider == filtered_user_by_email[0].auth_provider:
            register_user = authenticate(
                email=email, password=config('SECRET_KEY')
            )
            serializer = SocialTokenObtainPainSerializer(data={'email': email, 'password': config('SECRET_KEY')})
