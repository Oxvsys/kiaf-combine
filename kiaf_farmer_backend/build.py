...
private_key = pk_obj.private_bytes(encoding=serialization.Encoding.PEM,
                                   format=serialization.PrivateFormat.TraditionalOpenSSL,
                                   encryption_algorithm=serialization.NoEncryption())

public_key = pk_obj.public_key().public_bytes(encoding=serialization.Encoding.PEM,
...
env_options = [
    ...
    {"key": "GOOGLE_CLIENT_ID", "label": '<use: google_client_id for google authentication>'},
    {"key": "GOOGLE_CLIENT_SECRET", "label": '<use: google_client_secret for google authentication'},
    {"key": "TWITTER_API_KEY", "label": 'use: twitter_api_key for twitter authentication'},
    {"key": "TWITTER_CONSUMER_SECRET", "label": 'use: twitter_consumer_secret for twitter authentication'},
    ...
    {"key": "OTP_SERVICE_API_KEY_PROPERTY", "label": '<sms key to send key https://2factor.in/>'},
]
flag_env_exist = True
try:
    f = open(".env")
...
if flag_env_exist:
    value = input('.env file is already exist! Do you want to overwrite it (Y/N): ').lower()
    if value == 'y':
        create_env()
    else:
        print('Exiting...!')
